  ...
  let typecheck_function (fdef: fun_def) =
    ...
    let rec type_expr = function
      ...
      | Add(e1, e2) | Mul (e1, e2) |Div (e1, e2) |Mod (e1, e2) |Sub (e1, e2) -> 
        ...
        if t1 <> t2 || t1 <> Int then failwith "Integer operand was given a non integer expression : error"
        else Int
      ...
      | Or (e1, e2) | And (e1, e2) | Xor (e1, e2) -> 
        let t1 = type_expr e1 in let t2 = type_expr e2 in
        if t1 <> t2 || t1 <> Bool then failwith "Boolean operand was given a non boolean expression : error"
        else Bool
      ...
      | Get i ->  let errmsg = "Undefined variable \""^i^"\" : error" in
        begin 
          ...
          with Not_found -> 
            try let t, v = Hashtbl.find global_env i in if v = Undef then failwith "Variable not initialised : error " else t
